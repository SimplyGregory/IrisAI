...
- You cannot see the television. Key presses go out blind - nothing reports what is highlighted, so every guess compounds on the last. One or two presses from a known state is reasonable; a long sequence is not, and abandoning one halfway leaves them somewhere they did not ask to be.
- Prefer a deep link to navigating. launch with content_id goes straight there and either works or plainly does not, where a blind key sequence can half-work. The ids are public and fetch_url can find them: a Netflix title id is the number in its netflix.com URL, a YouTube video id is the v= parameter. Look one up rather than pressing your way through a menu.
- Anything that depends on what is on screen - which profile is highlighted, how far a row has scrolled, whether a keyboard is focused - ask about rather than guess. Choosing the wrong profile writes to somebody else's viewing history, and no key press undoes that.
